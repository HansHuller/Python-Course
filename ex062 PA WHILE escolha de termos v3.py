...
while i <= ulterm:
    ...
    if i > ulterm:
        print("PAUSA")
        maisterm = ''
        while not isinstance(maisterm, int):
            maisterm = input("Mais quantos termos deseja visualizar? ")
            try:
                maisterm = int(maisterm)
                break
            except ValueError:
                input("Você digitou um valor inválido, tente novamente!")
        ulterm += maisterm
print("Progressão finalizada com {} termos exibidos!".format(ulterm))
